 ...
 * counting.value(n) – устанавливает значение счетчика, возвращает 
 * новое значение
 ...
 * #3
 *
 * Создайте функцию myPow(a, b, myPrint). Внутри реализуйте 
 * рекурсию для подсчета результата возведения числа a в степень b.
 * функция myPrint(a, b, res) – глобальная функция, которая должна
 *  генерировать из параметров a, b, res строку вида 'a^b=res' 
 * и возвращать ее
 * myPrint() должна быть передана в myPow() как параметр и 
 * вызвана внутри как callback-функция
...
var  myPow = function(a,b,callback) {
 ...
 name: 'Skoda',
 year: 2008, 
 get used (){  return this.year===nowYear ? 'new':'used'},
 set used (value){ if  (value=== 'new' && this.year < nowYear) this.year = nowYear},
 info: fullInfo,
 };
 ...
 name: 'Toyota',
 year: 2022,
 //used: (car2.year===nowYear)? 'new':false ||'used';
 info: fullInfo,
 get used (){  return this.year===nowYear ? 'new':false ||'used'},
 set used (value){ if  (value=== 'new' && this.year < nowYear) this.year = nowYear},
 };
 ...
 * car.used – строка для описания состояния авто, допускаются 
 * значения 'used' и 'new'
 *
 * #5
 * Для созданных ранее объектов определите метод info(), используя 
 * ключевое слово this.
 * данный метод должен формировать и возвращать строку с полной 
 * информацией об автомобиле, например:
 ...
 * #6
 *
 * Для созданных ранее объектов измените свойство used, используя 
 * аксессоры (геттер и сеттер).
 * - используйте текущий год либо непосредственно в своем коде, 
 * либо с помощью глобальной переменной, например, yearNow
 * - если год выпуска автомобиля отличается от текущего года,
 *  геттер used должен выводить текст 'used'
 * - если год выпуска автомобиля совпадает с текущим годом, 
 * геттер used должен выводить текст 'new'
 * - если сеттеру used присвоено значение 'new', при этом года 
 * выпуска автомобиля отличается от текущего года,
 * - необходимо изменить год выпуска автомобиля, установив в 
 * качестве значения текущий год
 * - если сеттеру used присвоено значение 'used', ничего делать 
 * не нужно
 */

// let yearNow = new Date().getFullYear(); // получить текущий
 //год как число

  console.log(car.info()); // Chevrolet Lacetti, 2000cc, year
  2010//, used
...
//   return max;
// };
//  let list = [12, 23, 100, 34, 56, 9, 233,455, 715];
//  console.log(myMax(list)); // 233
 ...
 var myMax = function(list){
       ...
       let list = [12, 23, 100, 34, 56, 9, 233,455,777];
       console.log(myMax(list)); // 233
...
// // аналогичным образом создайте функцию myTriple(n), 
// //которая утраивает принимающий параметр, возвращая результат.
...
let myUniq = function (arrey){
let set = new Set();
arrey.forEach((value) => {set.add(value);
});
return set;
};
